...
from jakal_search.types import ExpansionMetrics
...
def test_train_branch_head_and_load_runtime_model(local_tmp_path: Path) -> None:
    examples = build_expansion_examples(
        embedder_kind="synthetic",
        model_name="unused",
        device="cpu",
    ...
    model = ExpansionDecisionModel(ScoringConfig(mlp_model_path=str(model_path)))
    positive = ExpansionMetrics(0.9, 0.8, 0.8, 0.6, 0.8, 1.0, 0.1)
    negative = ExpansionMetrics(0.1, 0.2, 0.2, 0.2, 0.1, 0.3, 0.8)
