...
from .scoring import ExpansionDecisionMLP, expansion_metrics_to_vector
...
from .types import ExpansionMetrics, SearchRequest, SearchRun, SearchTopic, TopicMemoryItem
from .utils import mean_embedding


@dataclass(slots=True)
class ExpansionExample:
...
@dataclass(slots=True)
class ExpansionTrainingMetrics:
    train_size: int
    val_size: int
    threshold: float
    val_accuracy: float
...
def _make_engine(provider, embedder, config, query: str):
    ...
    root = SearchTopic(topic_id="root", query=query, depth=0, centroid=query_vector, status="expanded")
    run = SearchRun(root_topic_id="root", request=SearchRequest(query=query))
    run.add_topic(root)
    return engine, run, root


def _append_expansion_example(
    examples: list[ExpansionExample],
    *,
    engine,
    run: SearchRun,
    root: SearchTopic,
    ...
    expansion = engine._evaluate_branch(run, root, docs, centroid)
    examples.append(ExpansionExample(metrics=expansion.metrics, label=label, source_case=source_case, topic_name=topic_name))
...
def _synthetic_example(bucket: list[ExpansionExample], *, label: int, rng: np.random.Generator) -> ExpansionExample:
    anchor = bucket[int(rng.integers(0, len(bucket)))]
    peer = bucket[int(rng.integers(0, len(bucket)))]
    alpha = float(rng.uniform(0.25, 0.75))
    base = (alpha * expansion_metrics_to_vector(anchor.metrics)) + ((1.0 - alpha) * expansion_metrics_to_vector(peer.metrics))
...
def _rebalance_examples(examples: list[ExpansionExample], *, target_count: int, seed: int) -> list[ExpansionExample]:
    ...
    balanced: list[ExpansionExample] = []
...
def build_expansion_examples(
    *,
    ...
    for case in BENCHMARK_CASES:
        ...
        engine, run, root = _make_engine(provider, embedder, config, case.query)
        ...
        for topic_name, group_docs in positive_groups:
            ...
            engine._topic_memory.append(TopicMemoryItem(topic_id=f"{case.name}:{topic_name}", embedding=centroid))
...
def stratified_split(examples: list[ExpansionExample], *, val_ratio: float, seed: int) -> tuple[list[ExpansionExample], list[ExpansionExample]]:
...
def train_expansion_head(
    ...
    *,
    ...
    val_ratio: float,
    seed: int,
) -> tuple[Path, ExpansionTrainingMetrics]:
    train_examples, val_examples = stratified_split(examples, val_ratio=val_ratio, seed=seed)
    train_x = np.asarray([expansion_metrics_to_vector(example.metrics) for example in train_examples], dtype=np.float32)
    val_x = np.asarray([expansion_metrics_to_vector(example.metrics) for example in val_examples], dtype=np.float32)
    ...
    model = ExpansionDecisionMLP(train_x.shape[1], hidden_dim)
    ...
    threshold, eval_metrics = evaluate_expansion_model(model, val_x, val_y)
    metrics = ExpansionTrainingMetrics(
    ...
    model_path = output_dir / "expansion_head.pt"
...
def evaluate_expansion_model(model: ExpansionDecisionMLP, features: np.ndarray, labels: np.ndarray) -> tuple[float, ExpansionTrainingMetrics]:
    model.eval()
    with torch.no_grad():
    ...
    return best_threshold, ExpansionTrainingMetrics(
        train_size=0,
        val_size=len(labels),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train an expansion decision head from the local benchmark suite.")
    ...
    parser.add_argument("--dataset-out", type=Path, default=Path("outputs") / "datasets" / "expansion_dataset.jsonl")
    ...
    examples = build_expansion_examples(
    ...
    model_path, metrics = train_expansion_head(
        examples,
        args.output_dir,
...
ExpansionDecisionExample = ExpansionExample

BranchExample = ExpansionExample
BranchTrainingMetrics = ExpansionTrainingMetrics
build_branch_examples = build_expansion_examples
train_branch_head = train_expansion_head
evaluate_branch_model = evaluate_expansion_model
